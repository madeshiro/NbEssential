...
public class Status {
    ...
    private static final HashMap<Integer, Status.ClientBound> CLIENT_BOUND_HASH;
    private static final HashMap<Integer, Status.ServerBound> SERVER_BOUND_HASH;

    static {
        CLIENT_BOUND_HASH = new HashMap<>();
        for (Status.ClientBound bound : Status.ClientBound.values())
            CLIENT_BOUND_HASH.put(bound.getID(), bound);

        SERVER_BOUND_HASH = new HashMap<>();
        for (Status.ServerBound bound : Status.ServerBound.values())
            SERVER_BOUND_HASH.put(bound.getID(), bound);
    }

    public static Status.ClientBound getClientPacket(int id) {
        return CLIENT_BOUND_HASH.get(id);
    }

    public static Status.ServerBound getServerPacket(int id) {
