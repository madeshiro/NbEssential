...
public class Login {
    ...
    private static final HashMap<Integer, Login.ClientBound> CLIENT_BOUND_HASH;
    private static final HashMap<Integer, Login.ServerBound> SERVER_BOUND_HASH;

    static {
        CLIENT_BOUND_HASH = new HashMap<>();
        for (Login.ClientBound bound : Login.ClientBound.values())
            CLIENT_BOUND_HASH.put(bound.getID(), bound);

        SERVER_BOUND_HASH = new HashMap<>();
        for (Login.ServerBound bound : Login.ServerBound.values())
            SERVER_BOUND_HASH.put(bound.getID(), bound);
    }

    public static Login.ClientBound getClientPacket(int id) {
        return CLIENT_BOUND_HASH.get(id);
    }

    public static Login.ServerBound getServerPacket(int id) {
